...
void display(int n) {
    printf("Detail of data:\n");
    printf("Location\tCity\t\t\tPrice\t\tRooms\tBathroom\tCarpark\tType\tFurnish\n");

    for (int i = 1; i < n; i++) {
        printf("%s%-15%s%-15%d%-15%d%-15%d%-15%d%-15%s%-15s\n",
...
void sort_by(const char *column, const char *order, int total_rows) {
    // Make a temporary array for sorting
    struct Data tempData[total_rows];
    for (int i = 0; i < total_rows; i++) {
        tempData[i] = searchData[i];
    }

    // Perform Merge Sort
    mergeSort(tempData, 0, total_rows - 1, column);

    printf("Data found. Detail of data:\n");
    printf("Location\tCity\t\t\tPrice\t\tRooms\tBathroom\tCarpark\tType\tFurnish\n");

    int displayRows = (total_rows < 10) ? total_rows : 10;
    ...
    do {
        ...
        switch (choice) {
            ...
            case 3: {
                ...
                sort_by(sortColumn, sortOrder, totalRows);
                break;
            }
            case 4: {
