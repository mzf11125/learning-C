...
void check_move(int letter1, int number1, int letter2, int number2, int letter3, int number3, int total){
	...
	if(absolute_value(letter1 - letter2) < absolute_value(letter3 - letter2) && absolute_value(letter1 - letter2) < absolute_value(letter3 - letter2)){
		return;	
	}
	for(int i = 0; i < 8; i++){
		if(result==true){ //Result is true, so break the for loop
			...
			check_move(letter1, number1, letter2, number2, letter3 + letter_move[i], number3 + number_move[i], total-1);//Recusrion
		}
	}            
}
